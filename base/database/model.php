    ...
    public function get() {
        ...
        if ( ! empty( $this->wheres ) ) {
            ...
            foreach ( $this->wheres as $where ) {
                $column   = preg_replace( '/[^a-zA-Z0-9_]/', '', $where['column'] );
                $operator = in_array( $where['operator'], $allowed_operators, true ) ? $where['operator'] : '=';
                // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                $conditions[] = $wpdb->prepare( "{$column} {$operator} %s", $where['value'] );
            }

            $sql .= ' WHERE ' . implode( ' AND ', $conditions );
        }

        // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, PluginCheck.Security.DirectDB.UnescapedDBParameter -- $sql is built from regex-sanitized column names, whitelisted operators, and $wpdb->prepare() for values.
