<?php
namespace Optiontics\Database;

use Optiontics\Database;
use WP_Post;
use Exception;
...
abstract class Post_Model implements JsonSerializable {
    ...
    public function __construct( $post = null ) {
        ...
        if ( $this->post && $this->post->post_type !== $this->get_post_type() ) {
            throw new Exception( esc_html__( 'Post type mismatch.', 'optiontics' ) );
        }
    ...
    public function __get( $key ) {
        ...
        throw new Exception( sprintf( esc_html__( 'Undefined property %s', 'optiontics' ), esc_html( $key ) ) );
    ...
    public static function paginate( $args = [] ) {
        ...
        if ( isset( $args['search'] ) ) {
            // Use WP_Query's native 's' parameter so post_title is searched.
            if ( ! empty( $args['search'] ) ) {
                $args['s'] = $args['search'];
            }
            // Also apply a meta_query if the concrete model implements search_query().
            $meta_search = ( new static )->search_query( $args['search'] );
    ...
    public static function create( array $attributes ) {
        if ( ! self::validate_attributes( $attributes ) ) {
            throw new Exception( esc_html__( 'Invalid property provided.', 'optiontics' ) );
        }
    ...
    public function update( array $attributes ) {
        if ( ! $this->post ) {
            throw new Exception( esc_html__( 'Cannot update a non-existent post.', 'optiontics' ) );
        }
    ...
    protected static function validate_attributes( $attributes = [] ) {
        ...
        foreach ( $attributes as $key => $value ) {
            if ( ! isset( (new static)->fillable[$key] ) ) {
                // translators: %s is the name of the invalid property.
                throw new Exception( sprintf( esc_html__( 'Invalid property provided: %s', 'optiontics' ), esc_html( $key ) ) );
            }
        }
