...
namespace Optiontics\Container;

use Exception;
use ReflectionClass;
use ReflectionException;
use ReflectionParameter;
use Optiontics\Container\Exception\Dependency_Has_No_Default_Value_Exception;
use Optiontics\Container\Exception\Dependency_Is_Not_Instantiable_Exception;
...
class Container {
    ...
    public function get( string $key ): object {
        if ( ! $this->has( $key ) ) {
            ...
            throw new \Exception( sprintf( esc_html__( 'Service provider %s not found.', 'optiontics' ), esc_html( $key ) ) );
    ...
    public function resolve( string $class_name ): object {
        if ( ! class_exists( $class_name ) ) {
            // translators: %s is the name of the class that is not instantiable.
            throw new Exception( sprintf( esc_html__( 'Class: %s does not exist.', 'optiontics' ), esc_html( $class_name ) ) );
        ...
        if ( ! $reflection_class->isInstantiable() ) {
            throw new Dependency_Is_Not_Instantiable_Exception(
                // translators: %s is the name of the class that is not instantiable.
                sprintf( esc_html__( 'Class: %s is not instantiable.', 'optiontics' ), esc_html( $class_name ) )
            );
        }
            ...
            if ( is_null( $dependency ) ) {
                if ( $parameter->isDefaultValueAvailable() ) {
                    ...
                    throw new Dependency_Has_No_Default_Value_Exception(
                        // translators: %s is the name of the class that is not instantiable.
                        sprintf( esc_html__( 'Class: %s dependency cannot be resolved.', 'optiontics' ), esc_html( $parameter->name ) )
                    );
                }
            } else {
