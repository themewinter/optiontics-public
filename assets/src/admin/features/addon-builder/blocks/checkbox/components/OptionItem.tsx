    ...
    return (
        <div className="flex items-center justify-between px-3 group">
            <div
                ...
                onClick={() => handlers.toggle(index, !option.default)}
            >
                <Checkbox
                    checked={option.default || false}
                    onCheckedChange={checked => handlers.toggle(index, Boolean(checked))}
                />
            ...
            <div className="flex items-center gap-2">
                <If condition={nodeProps.isQuantity}>
                    <Input
                        ...
                        onChange={e => handlers.updateQuantity(index, Number(e.target.value))}
                    />
                </If>

                <Button size="icon" variant="ghost" className="h-8 w-8 opacity-0 group-hover:opacity-100 group-hover:bg-neutral-200 transition-opacity duration-300 opt-option-delete-btn" onClick={() => handlers.remove(index)}>
